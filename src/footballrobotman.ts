import {
  Client as discord, DMChannel, MessageEmbed, NewsChannel, TextChannel,
} from 'discord.js';
...
function getWeekNumber(now: Date): number {
  const millisElapsed: number = now.getMilliseconds() - SEASON_START_DATE.getMilliseconds();
...
export class FootballRobotMan {
  ...
  start(commandPrefix: string): void {
    this.discordClient.on('message', (message) => {
      this.logger.log('info', `${message.channel.id} - ${message.author.username}: ${message.content}`);
      ...
      if (message.content.toLowerCase().startsWith(commandPrefix)) {
        ...
        const command = message.content.substring(commandPrefix.length).trimStart();
    ...
    scheduleJob({ dayOfWeek: 2, hour: 9, minute: 0 }, () => {
      this.sendStandings(getWeekNumber(new Date()),
        this.discordClient.channels.cache.get(this.defaultChannelId) as TextChannel);
    });
  }
  ...
  private async sendStandings(
    ...
    const teams = await this.espnClient.getTeamsAtWeek({
      seasonId: this.seasonId, scoringPeriodId: weekNumber,
    });

    teams.sort(compareTeams);
    const lines = teams.map((element: Team) => `${element.wins}-${element.losses}: ${element.name}`).join('\n');

    const embed = new MessageEmbed().setTitle(`Week ${weekNumber} Standings`).addField('\u200b', lines);
